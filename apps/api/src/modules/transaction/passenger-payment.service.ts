...
export class PassengerPaymentService {
  ...
  private async executeScheduledBooking(
    ...
    this.logger.log(
      `Scheduled ride ${ride.rideUUId} booked: passenger -${amount}, driver +${driverAmount}, admin commission +${commissionAmount}`,
    );
      ...
      { _id: ride._id, rideStatus: RideStatus.BOOKING },
      {
        $set: {
          ...
          schedule: {
            ...
            timeSlots: (day.timeSlots || []).map((s: any) => ({
  ...
  private async processPaymentLogic(
    ...
    //TODO:passengers totalridesaspassenger
