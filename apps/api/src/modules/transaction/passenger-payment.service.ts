...
import {
  PaymentMethodEnum,
  PaymentStatusEnum,
} from "@libs/data-access/enums/payment.enum";
...
import {
  AdminUser,
  AdminUserDocument,
} from "@libs/data-access/entities/admin-user.entity";
import {
  ErrorException,
...
export class PassengerPaymentService {
  ...
  private async getBaatoRoute(
    ...
    try {
      ...
      const detail = error?.response?.data
        ...
        : error?.message;
      this.logger.error(
        `Baato API error: ${detail}. Using haversine fallback.`,
      );
      return this.haversineEstimate(originLat, originLng, destLat, destLng);
    }
  }
  ...
  private haversineEstimate(
    ...
    return {
      distanceKm,
      durationMinutes: this.estimateDurationMinutes(distanceKm),
    };
  }
  ...
  async bookScheduledRide(
    ...
    this.logger.log(
      `Booking scheduled ride ${rideId} with driver ${driverId} for passenger ${passengerId}`,
    );

    // ── Validate ride ─────────────────────────────────────────
    const ride = await this.ridesRepository.findById(
    ...
    if (ride.rideStatus !== RideStatus.BOOKING) {
      throw new BadRequestException(
        `Ride is not in BOOKING status. Current: ${ride.rideStatus}`,
      );
    }
    ...
    const day = availabilityDoc.days.find(
      (d) =>
        d &&
        this.utcStartOfDay(new Date(d.date)).getTime() ===
          targetDate.getTime() &&
        d.isAvailableForBookings !== false,
    );
    if (!day) {
      throw new BadRequestException(
        "Driver is not available on the ride booking date",
      );
    }
    ...
    if (perSeatAmount <= 0) {
      throw new BadRequestException(
        `No booking amount configured for the driver's availability day`,
      );
    }
    const amount = Number((perSeatAmount * seatsBooked).toFixed(2));
    if (amount <= 0) {
      throw new BadRequestException(
        `Invalid booking amount for the driver's availability day`,
      );
    }
    ...
    if (day.timeSlots?.length) {
      ...
      const matchesSlot = day.timeSlots.some((s) => {
        ...
        return (
          bookingMs >= startMs || bookingMs <= startMs - bufferMinutes * 60000
        );
      });
      if (!matchesSlot) {
        throw new BadRequestException(
          `Booking time does not match any of the driver's time slots`,
        );
      }
    }

    return this.executeScheduledBooking(
      ride,
      driverId,
      passengerId,
      day,
  ...
  private async executeScheduledBooking(
    ...
    const session = useTransactions
      ? await this.getSession(useTransactions)
      : null;
    const transactions: {
      transactionId: string;
      userId: string;
    ...
    try {
      const bookingLogic = async () => {
        ...
        await this.applyScheduledBookingToRide(
          ride,
          driverId,
          day,
          amount,
  ...
  private async applyScheduledBookingToRide(
    ...
    //TODO earning by driver
    const commissionAmount = Number((amount * commissionRate).toFixed(2));
    const driverAmount = Number((amount - commissionAmount).toFixed(2));

    //TODO totalridesas passenger
    const passenger = await this.userDetailsModel
      ...
      .exec();
    if (!passenger) {
      this.logger.debug(`Passener not found: ${passengerId}`);
    } else {
      this.logger.debug(
    ...
    if (!driver) {
      this.logger.debug(`Driver not found: ${ride.driverId}`);
    } else {
      this.logger.debug(
        `Driver stats updated | ` +
          `driverId=${ride.driverId} | ` +
          `rides: ${driver.totalRidesAsDriver - 1} → ${driver.totalRidesAsDriver} | ` +
          `earnings: ${driver.totalEarnings - driverAmount} → ${driver.totalEarnings} | ` +
          `earned=${driverAmount}`,
      );
    }
  ...
  private isUsingAtlas(): boolean {
    ...
    return (
      mongoUri.includes("mongodb.net") || mongoUri.includes(".mongodb.net")
    );
  }
  ...
  async processPayment(
    ...
    this.logger.log(
      `Processing payment for ride ${rideId} by passenger ${passengerId}`,
    );
    ...
    const driver = await this.userDetailsRepository.findOne({
      userId: ride.driverId,
    });
    if (!driver) {
    ...
    const fareBreakdown = await this.calculateFareBreakdown(ride, promoCodeId);
    this.logger.log(
      `Fare breakdown for ride ${rideId}: ${JSON.stringify(fareBreakdown)}`,
    );
    const admin = await this.adminModel.findOne().sort({ createdAt: 1 }).exec();
    this.adminId = admin?._id.toString() || "";
    const useTransactions = this.isUsingAtlas();
    ...
    try {
      ...
      if (!transaction) {
        this.logger.warn(
          `No driver credit transaction found for ride ${ride._id.toString()}, skipping earnings update`,
        );
      } else {
        ...
        if (!userDetails) {
          this.logger.warn(
            `User details not found for driver ${ride.driverId}, skipping earnings update`,
          );
        } else {
          this.logger.log(
  ...
  private async calculateFareBreakdown(
    ...
    if (promoCodeId) {
      ...
      if (
        new Date() < promoCode.startDateTime ||
        new Date() > promoCode.expiryDateTime
      ) {
      ...
      if (promoCode.minimumFare && subTotal < Number(promoCode.minimumFare)) {
        throw new BadRequestException(
          `Minimum fare of NPR ${promoCode.minimumFare} required`,
        );
      }
      ...
      // Calculate discount
      if (
        promoCode.discountType === DiscountTypeEnum.PERCENTAGE &&
        promoCode.percentageAmount
      ) {
        discount = Number(ride.fare?.discountAmount || 0);
      } else if (
        promoCode.discountType === DiscountTypeEnum.FLAT &&
        promoCode.flatAmount
      ) {
        discount = Number(ride.fare?.discountAmount || 0);
      }
    }
  ...
  private async processPaymentLogic(
    ...
    console.log(
      "Processing payment logic for ride:",
      ride._id.toString(),
      "Passenger:",
      passengerId,
      "Payment Method:",
    ...
    if (paymentMethod === PaymentMethodEnum.WALLET) {
      ...
      const driverAmount =
        fareBreakdown.totalFare - fareBreakdown.commissionAmount;
      await this.walletService.creditWallet(driverId, driverAmount, session);
    ...
    await this.ridesModel.updateOne(
      { _id: ride._id },
      {
        $set: {
          paymentDetails,
          isAcknowledgeByDriver:
            paymentMethod === PaymentMethodEnum.CASH ? false : true,
          rideStatus:
            paymentMethod === PaymentMethodEnum.CASH
              ? ride.rideStatus
              : "COMPLETED", // If cash, keep the ride status as is; if wallet, mark as completed
          rideCompletedAt:
            paymentMethod === PaymentMethodEnum.CASH
              ? ride?.rideCompletedAt || null
              : new Date(), // If cash, keep the ride completed time as is; if wallet, set to now
        },
      },
      { session },
    );
