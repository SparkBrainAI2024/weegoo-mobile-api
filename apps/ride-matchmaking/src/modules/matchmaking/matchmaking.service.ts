...
} from "@libs/data-access/entities/user-daily-online-status.entity";
import {
  Vehicle,
  VehicleDocument,
...
import {
  PromoCode,
  PromoCodeDocument,
} from "@libs/data-access/entities/promo-code.entity";
import {
  PromoCodeUsed,
...
import {
  ...
  VehicleType,
} from "@libs/data-access/enums/vehicle.enum";
import {
  roles,
  DriverOnlineStatus,
  ridePreference,
} from "@libs/data-access/enums/user.enum";
import { NotificationType } from "@libs/data-access/enums/notification.enum";
import {
  PromoCodeStatusEnum,
  DiscountTypeEnum,
  AppliedToEnum,
} from "@libs/data-access/enums/promo-code.enum";
...
import {
  MATCHMAKING_CONFIG,
  toMongoId,
  resolveBookedTimeSlots,
  resolveScheduledSeatPool,
  SCHEDULED_SEAT_FIELD,
  SCHEDULED_SEAT_POOL,
  resolveBookedSlotIndex,
  parseSlotStartTime,
} from "@libs/common";
import { getActiveProfileImageUrl } from "@libs/common/utils/entity.utils";
import { S3Service } from "@libs/s3";
import {
  resolveSlotTimeDiffMs,
  sortBySlotMatchThenLocation,
} from "./scheduled-driver-order.util";
...
export class MatchmakingService {
  ...
  async matchDrivers(params: { rideId: string }): Promise<MatchResult> {
    ...
    if (ride.rideType !== RideTypes.INSTANT)
      return {
        ...
        passengerId: ride.passengerId.toString(),
        attempts: [],
        message: "Use matchScheduledDrivers for SCHEDULED rides.",
      };
  ...
  async matchScheduledDrivers(params: { rideId: string }): Promise<{
    matched: boolean;
    rideId: string;
    rideUUId: string;
    passengerId: string;
    ...
    if (
      ...
      ride.rideStatus !== RideStatus.BOOKING
    )
      return {
        matched: false,
        rideId,
        ...
        passengerId: ride.passengerId.toString(),
        attempts: [],
        message: `Ride is not in BOOKING/PENDING status. Current: ${ride.rideStatus}`,
      };
    if (ride.rideType !== RideTypes.SCHEDULED)
      return {
        ...
        passengerId: ride.passengerId.toString(),
        attempts: [],
        message: "Use matchDrivers for INSTANT rides.",
      };
  ...
  private async executeExpandingRingMatch(
    ...
    const passengerUser = await this.userModel
      .findById(ride.passengerId)
      .exec();
    const passengerDetails = await this.userDetailsModel
      .findOne({ userId: ride.passengerId })
      .exec();
    const passengerName =
      passengerUser?.fullName || passengerDetails?.fullName || "Passenger";
    const passengerPhone = passengerUser?.phone || "";
    const passengerProfileImages =
      passengerDetails?.profileImages
        ?.map((img) =>
          getActiveProfileImageUrl([img], (key) => this.s3.getPublicUrl(key)),
        )
        .filter(Boolean) || [];
    ...
    const pickupLng = pickupCoords[0];
    const vehicle =
      ride.vehicle || (await this.vehicleModel.findById(ride.vehicleId).exec());
    ...
    this.logger.log(
      `Calculated route for ride ${ride.rideUUId}: distance ${routeDistanceKm} km, duration ${routeDurationMinutes} minutes`,
    );
    ...
    await this.ridesModel
      .findByIdAndUpdate(ride._id, {
        $set: {
    ...
    for (
      let attemptIdx = 0;
      attemptIdx < radii.length && !matched;
      attemptIdx++
    ) {
      ...
      if (!attemptStartRide) {
        this.logger.log(
          `Ride ${ride.rideUUId} was deleted during matchmaking (attempt-start check). Aborting.`,
        );
        return {
          matched: false,
          rideId,
      ...
      if (attemptStartRide.rideStatus !== RideStatus.PENDING) {
        if (attemptStartRide.rideStatus === RideStatus.CANCELLED) {
          this.logger.log(
            `Ride ${ride.rideUUId} was cancelled by user (attempt-start check). Aborting matchmaking.`,
          );
          return {
            matched: false,
            rideId,
      ...
      this.logger.log(
        `[INSTANT] Attempt ${attemptIdx + 1}: Searching drivers within ${radiusKm} km`,
      );
      ...
      const filteredDrivers = drivers.filter(
        (d) => !respondedDriverIds.has(d.driverId),
      );
      ...
      for (const driver of requestBatch) {
        ...
        if (!currentRide) {
          this.logger.log(
            `Ride ${ride.rideUUId} was deleted during matchmaking. Aborting.`,
          );
          return {
            matched: false,
            rideId,
        ...
        if (currentRide.rideStatus !== RideStatus.PENDING) {
          if (currentRide.rideStatus === RideStatus.CANCELLED) {
            this.logger.log(
              `Ride ${ride.rideUUId} was cancelled by user. Aborting matchmaking.`,
            );
            return {
              matched: false,
              rideId,
        ...
        if (driverUser) {
          const ablyChannelId =
            ride.ablyChannelId || `WG-RIDE-${ride.rideUUId}-ride-details`;
          this.logger.log(
            `Sending ride request notification to driver ${driver.driverId} with ${waitTimeSeconds}s to respond`,
          );
          ...
          try {
            ...
            this.logger.error(
              `Failed to send notification to driver ${driver.driverId}: ${err}`,
            );
        ...
        const preSubscribeRideCheck = await this.ridesModel
          .findById(ride._id)
          .exec();
        if (!preSubscribeRideCheck) {
          this.logger.log(
            `Ride ${ride.rideUUId} was deleted during matchmaking (pre-subscribe check). Aborting.`,
          );
          return {
            matched: false,
            rideId,
        ...
        if (preSubscribeRideCheck.rideStatus !== RideStatus.PENDING) {
          if (preSubscribeRideCheck.rideStatus === RideStatus.CANCELLED) {
            this.logger.log(
              `Ride ${ride.rideUUId} was cancelled by user (pre-subscribe check). Aborting matchmaking.`,
            );
            return {
              matched: false,
              rideId,
        ...
        const { promise: driverResponsePromise, unsubscribe } =
          this.subscribeForDriverResponse(
            ride.rideUUId,
            [driver.driverId],
        ...
        const postResponseRide = await this.ridesModel
          .findById(ride._id)
          .exec();
        if (!postResponseRide) {
          this.logger.log(
            `Ride ${ride.rideUUId} was deleted during matchmaking (post-response check). Aborting.`,
          );
          return {
            matched: false,
            rideId,
        ...
        if (postResponseRide.rideStatus !== RideStatus.PENDING) {
          if (postResponseRide.rideStatus === RideStatus.CANCELLED) {
            this.logger.log(
              `Ride ${ride.rideUUId} was cancelled by user (post-response check). Aborting matchmaking.`,
            );
            return {
              matched: false,
              rideId,
        ...
        if (driverResponse.accepted) {
          ...
          const acceptedDriver = requestBatch.find(
            (d) => d.driverId === driverResponse.driverId,
          );
          ...
          const acceptDetails = await this.buildAcceptDetails(
            ride,
            acceptedDriverId,
            estimatedFare,
          );
          return {
            ...
            driverImage: acceptedDriverImage,
            rating: acceptedRating,
            estimatedFare,
            attempts,
      ...
      const timeoutExpired = lastDriverResponse
        ? !lastDriverResponse.accepted
        : true;
      attempts.push({
        attemptNumber: attemptIdx + 1,
        radiusKm,
        ...
        driverAccepted,
        acceptedDriverId: acceptedDriverIdForAttempt,
        timeoutExpired,
        status: driverAccepted ? "accepted" : "timeout",
      });
    }
    if (!matched) {
      const failMessage =
  ...
  private async findAvailableDrivers(
    ...
    const vehicles = await this.vehicleModel
      .find({ vehicleType: vehicleType as VehicleType })
      .populate("driverId")
    ...
    const driverIds = validVehicles
      ...
      .filter(Boolean);
    this.logger.log(
      `Found ${driverIds.length} drivers for vehicle type ${vehicleType} in attempt ${attemptIndex + 1}`,
    );
    ...
    this.logger.log(
      `Checking active rides for ${activeRideDriverIds.length} online drivers`,
    );
    const activeRides =
      activeRideDriverIds.length > 0
        ? await this.ridesModel
            .find({
            ...
            .exec()
        : [];
    const activeRideDriverIdSet = new Set(
      activeRides.map((r) => r.driverId.toString()),
    );

    // Batch-fetch completed trip counts for all drivers in one aggregation
    const completedCounts = await this.ridesModel
      .aggregate([
        {
          $match: {
    ...
    for (const v of validVehicles) {
      ...
      this.logger.log(
        `Checking driver ${driver._id} for availability: loginAs=${driver?.loginAs}, suspended=${driver?.suspended}, verified=${driver?.verified}`,
      );
      ...
      if (
        userDetails.geoLocation?.coordinates &&
        userDetails.geoLocation.coordinates.length >= 2
      ) {
      ...
      if (distResult.distanceKm <= radiusKm) {
        const completedTripsCount =
          completedCountsMap.get(driver._id.toString()) || 0;
        drivers.push({
          driverId: driver._id.toString(),
          fullName: userDetails?.fullName || driver.fullName || "Driver",
          phone: driver.phone || "",
  ...
  private resolveAvailabilityDay(
    ...
    const seatPool = resolveScheduledSeatPool(
      bookingTime,
      day.timeSlots,
      day.isOneWay,
    );
    const configuredSeats =
      Number((day as any)[SCHEDULED_SEAT_FIELD[seatPool]]) || 0;
  ...
  private buildScheduledAvailabilityInfo(
    ...
    return {
      ...
      remainingSeats:
        remainingSeats != null
          ? Math.max(0, Math.floor(remainingSeats))
          : undefined,
      ...
      pickupLocation: day.pickupLocation
        ? {
            ...
            latitude: day.pickupLocation.latitude,
            longitude: day.pickupLocation.longitude,
          }
        : null,
      dropOffLocation: day.dropOffLocation
        ? {
            address: day.dropOffLocation.address || "",
            latitude: day.dropOffLocation.latitude,
            longitude: day.dropOffLocation.longitude,
          }
        : null,
  ...
  private async findAvailableScheduledDrivers(
    ...
    const vehicles = await this.vehicleModel
      .find({
        vehicleType: { $in: scheduledVehicleTypes as string[] },
        deleted: false,
      })
      .populate("driverId")
      .limit(MATCHMAKING_CONFIG.MAX_DRIVERS_PER_RING)
    ...
    const driverIds = vehicles
      ...
      .filter(Boolean);
    this.logger.log(
      `Found ${driverIds.length} drivers of scheduled vehicle types for booking search`,
    );
    const userDetailsDocs = await this.userDetailsModel
      .find({ userId: { $in: driverIds }, deleted: false })
      .exec();
    const userDetailsMap = new Map<string, UserDetailsDocument>();
    for (const ud of userDetailsDocs) {
      userDetailsMap.set(ud.userId.toString(), ud);
    ...
    this.logger.log(
      `Fetching availability & booked-seat counts for ${driverIds.length} drivers`,
    );
    ...
    const availabilityDocs = await this.availabilityModel
      .find({ driverId: { $in: driverIds }, deleted: false })
      .exec();
    ...
    for (const v of vehicles) {
      ...
      const resolvedDay = this.resolveAvailabilityDay(
        availabilityDoc,
        bookingReference,
        noOfPassengers,
      );
      this.logger.log(
        `Driver ${driver._id} availability check for ${bookingReference.toISOString()}: resolvedDay=${resolvedDay ? "YES" : "NO"}`,
      );
      ...
      this.logger.log(
        `Driver ${driver._id} has ${remainingSeats} remaining seats for ${bookingReference.toISOString()}`,
      );
      ...
      if (!dayDrop || dayDrop.latitude == null || dayDrop.longitude == null)
        continue;
      if (dropoffLat != null && dropoffLng != null) {
        const destDistanceKm = haversineKm(
          dropoffLat,
          dropoffLng,
          dayDrop.latitude,
          dayDrop.longitude,
      ...
      if (
        !dayPickup ||
        dayPickup.latitude == null ||
        dayPickup.longitude == null
      )
        continue;
      ...
      const slotIndex = resolveBookedSlotIndex(
        bookingReference,
        day.timeSlots || [],
      );
      const matchedSlot =
        slotIndex >= 0 ? (day.timeSlots || [])[slotIndex]?.startTime : null;
      const matchedSlotStart = matchedSlot
        ? parseSlotStartTime(String(matchedSlot), bookingReference)
        : null;
      const slotTimeDiffMs = resolveSlotTimeDiffMs(
        bookingReference,
        matchedSlotStart,
      );
      this.logger.log(
      ...
      try {
        const pickupDist =
          await this.distanceCalculator.calculateDriverDistance(
            pickupLat,
            pickupLng,
            dayPickup.latitude,
            dayPickup.longitude,
            (
              (day.vehicleType as string) ||
              v.vehicleType ||
              "CAR"
            ).toLowerCase(),
          );
        ...
        candidates.push({
          slotTimeDiffMs,
          distanceToPickupKm: pickupDist.distanceKm,
          driver: {
            driverId: driver._id.toString(),
            fullName: userDetails?.fullName || driver.fullName || "Driver",
            phone: driver.phone || "",
            ...
            numberPlate: v.numberPlate,
            year: v.year ?? null,
            isAcType: v.isAcType ?? (v.vehicleType as string) === "CAR",
            vehicleModelType: v.vehicleModelType || null,
            vehicleImage: v.images?.length
              ? this.s3.getPublicUrl(
                  v.images.find((img) => img.status === "ACTIVE")?.s3Key ||
                    v.images[0].s3Key,
                )
              : null,
            distanceToPickupKm: pickupDist.distanceKm,
            ...
            completedTripsCount,
            score: 0,
            estimatedTimeToReachMinutes: pickupDist.durationMinutes,
            scheduledAvailability: this.buildScheduledAvailabilityInfo(
              day,
              resolvedDay.effectiveSeats,
              remainingSeats,
              completedTripsCount,
              bookingReference,
            ),
        ...
        this.logger.warn(
          `Failed to compute pickup distance for driver ${driver._id}: ${err}`,
        );
      }
    }
    ...
    const orderedDrivers = sortBySlotMatchThenLocation(candidates).map(
      (c) => c.driver,
    );
    this.logger.log(
      `Found ${orderedDrivers.length} available scheduled drivers for the requested destination`,
    );
  ...
  private scoreDrivers(drivers: DriverScore[]): DriverScore[] {
    ...
    const maxDistance = Math.max(
      ...drivers.map((d) => d.distanceToPickupKm),
      1,
    );
    const maxRating = 5.0;
   ...
   * publishing client.
   */
  private pendingDriverResponseResolvers = new Map<
    string,
    (value: {
      accepted: boolean;
      driverId?: string;
    ...
    const handler = (message: any) => {
      ...
      if (response.eventType === "driver-response") {
        if (
          ...
          driverIds.includes(response.driverId) &&
          !rejectedDriverIds.includes(response.driverId)
        ) {
  ...
  async handleDriverResponse(
    ...
    driverId: string,
    action: "accept" | "reject",
  ): Promise<{ success: boolean; message: string; acceptedDetails?: any }> {
    try {
      ...
      if (action === "accept") {
        ...
        const [driverUser, vehicle, driverDetails, passengerUser] =
          await Promise.all([
        ...
        this.logger.log(
          `Driver ${driverId} is ${driverToPickupDistanceKm} km away from pickup, ETA ${driverToPickupDurationMinutes} minutes`,
        );
        ...
        if (
          updatedRide.rideType === RideTypes.SCHEDULED &&
          ...
          try {
            ...
            const resolved = this.resolveAvailabilityDay(
              availabilityDoc,
              updatedRide.bookingTime,
              noOfPassengers,
            );
            if (resolved) {
              ...
              await this.ridesModel
                .findByIdAndUpdate(updatedRide._id, {
                  $set: {
                    schedule: {
                      ...currentSchedule,
                      ...
                      pickupBufferTimeMinutes:
                        resolved.day.pickupBufferTimeMinutes || 0,
            ...
            this.logger.warn(
              `Failed to enrich schedule for ride ${updatedRide.rideUUId}: ${err}`,
            );
          }
        }
        ...
        if (passengerUser) {
          ...
          const notificationInput: CreateNotificationInput = {
            ...
            description:
              "Your ride request has been accepted by a driver. They are on their way to pick you up!",
            ablyChannelId,
            driverName:
              acceptDetails?.driver?.fullName || driverName || "Driver",
            driverPhone:
              acceptDetails?.driver?.phone || driverUser?.phone || "",
            driverProfileImage: acceptDetails?.driver?.profileImage || null,
            driverRating: acceptDetails?.driver?.rating || null,
            vehicleType:
              acceptDetails?.vehicle?.vehicleType ||
              vehicle?.vehicleType ||
              null,
            vehicleModel:
              acceptDetails?.vehicle?.vehicleModel ||
              vehicle?.vehicleModel ||
              null,
            vehicleColor:
              acceptDetails?.vehicle?.color || vehicle?.color || null,
            vehicleNumberPlate:
              acceptDetails?.vehicle?.numberPlate ||
              vehicle?.numberPlate ||
              null,
            ...
            estimatedFare:
              acceptDetails?.estimatedFare || ride.estimatedFare || null,
            estimatedTimeInMinutes:
              driverToPickupDurationMinutes ||
        ...
        await this.subscribeToDriverLocationChannel(driverId)
          .then((res: any) => {
            this.logger.log(
              `Successfully subscribed driver ${driverId} to location channel`,
            );
            ...
            if (driverToPickupDistanceKm <= 0.3) {
              ...
              this.rideChannelService
                .publishDriverLocationToChannel(driverId, {
                  driverId,
                  latitude: driverLat,
                  longitude: driverLng,
                ...
                .catch((err) =>
                  this.logger.warn(
                    `Background driver location publish to driver channel failed: ${err}`,
                  ),
                );
            }
          })
    ...
    if (
      ...
      try {
        const route = await this.distanceCalculator.calculateDistance(
          ...
          dropoffCoords[1],
          dropoffCoords[0],
          vehicleTypeNew?.toLowerCase() || "car",
        );
  ...
  async getScheduledEstimatedFare(
    rideId: string,
    rain?: RainCondition,
    historicalTraffic?: HistoricalTraffic,
  ): Promise<ScheduledFareBreakdown | null> {
    const ride = await this.ridesModel
      .findById(new Types.ObjectId(rideId))
      .exec();
    if (!ride) return null;
    ...
    if (
      ride.rideType === RideTypes.SCHEDULED &&
      (ride.estimatedFare || 0) > 0
    ) {
      return {
    ...
    if (
      ...
      try {
        ...
        this.logger.log(
          `Failed to calculate distance for scheduled fare estimation, using defaults for ride ${rideId}`,
        );
      }
    }
    return this.pricingService.calculateScheduledFare({
      distanceKm,
      durationMinutes,
      vehicleType,
    });
  ...
  async startRide(
    ...
    try {
      ...
      if (ride.rideStatus !== RideStatus.PICKUP)
        return {
          success: false,
          message: `Driver must pickup passenger (PICKUP) before starting ride. Current: ${ride.rideStatus}`,
        };
      ...
      const updatedRide = await this.ridesModel
        ...
        .exec();
      if (!updatedRide)
        return { success: false, message: "Failed to update ride status" };
        ...
        await this.notificationService.createNotification(
          {
            ...
            ablyChannelId:
              updatedRide.ablyChannelId ||
              `WG-RIDE-${ride.rideUUId}-ride-details`,
      ...
      await this.subscribeToDriverLocationChannel(driverId)
        .then(() => {
          this.logger.log(
            `Successfully subscribed driver ${driverId} to location channel on ride start`,
          );
        })
        .catch((err: any) =>
          this.logger.warn(
  ...
  private async processDriverLocationForRides(
    ...
    for (const activeRide of activeRides) {
      ...
      if (pickupCoords && pickupCoords.length >= 2) {
        ...
        await this.ridesModel
          .findByIdAndUpdate(activeRide._id, {
            $set: {
        ...
        if (
          !isScheduledRide &&
          activeRide.rideStatus === RideStatus.CONFIRMED &&
          distanceKm <= 0.3 &&
          !activeRide.driverArrivingNotified
        ) {
          await this.ridesModel
            .findByIdAndUpdate(activeRide._id, {
              $set: { driverArrivingNotified: true },
            })
            .exec();
          const passenger = await this.userModel
            .findById(activeRide.passengerId)
            .exec();
          if (passenger) {
            await this.rideChannelService.publishRideEvent(
              activeRide.rideUUId,
            ...
            this.notificationService.createNotification(
              {
                ...
                ablyChannelId:
                  activeRide.ablyChannelId ||
                  `WG-RIDE-${activeRide.rideUUId}-ride-details`,
        ...
        if (
          (activeRide.rideStatus === RideStatus.ONGOING ||
            activeRide.rideStatus === RideStatus.PICKUP) &&
          dropoffCoords &&
          dropoffCoords.length >= 2
        ) {
          const dropoffLat = dropoffCoords[1];
          ...
          if (
            dropoffDistanceKm > 0.05 &&
            !activeRide.driverArrivedAtDestinationNotified
          )
            await this.rideChannelService.publishRideEvent(
              activeRide.rideUUId,
              "driver-moving-destination",
              {
                rideId: activeRide._id.toString(),
                driverId,
                latitude,
          ...
          if (
            ...
            await this.ridesModel
              .findByIdAndUpdate(activeRide._id, {
                $set: { driverArrivedAtDestinationNotified: true },
              })
              .exec();
            const passenger = await this.userModel
              .findById(activeRide.passengerId)
  ...
  async subscribeToDriverLocationChannel(driverId: string): Promise<boolean> {
    if (this.driverLocationSubscriptions.has(driverId)) {
      this.logger.warn(
        `Driver ${driverId} already subscribed to location channel. Re-subscribing.`,
      );
      this.unsubscribeFromDriverLocationChannel(driverId);
    }

    const unsubscribe =
      this.rideChannelService.subscribeToDriverLocationChannel(
        driverId,
        async (data: any) => {
          ...
          await this.userDetailsModel
            .findOneAndUpdate(
              ...
              {
                $set: {
                  geoLocation: { type: "Point", coordinates: [lat, lng] },
          ...
          const vehicle = await this.vehicleModel
            .findOne({ driverId: driverObjectId, deleted: false })
            .exec();
    ...
    this.logger.log(
      `Subscribed to driver ${driverId} location channel for matchmaking`,
    );
    return true;
  }
   ...
   */
  async markStaleDriversOffline(): Promise<{
    processed: number;
    markedOffline: number;
    errors: number;
  }> {
  ...
  async pickupPassenger(
    ...
    this.logger.log(
      `Driver ${driverId} picked up passenger for ride ${rideId}`,
    );
    try {
      const ride = await this.ridesModel
        .findById(new Types.ObjectId(rideId))
      ...
      if (ride.rideStatus !== RideStatus.CONFIRMED)
        return {
          success: false,
          message: `Ride must be CONFIRMED before pickup. Current: ${ride.rideStatus}`,
        };
      ...
      if (dropoffCoords?.[1] && dropoffCoords?.[0]) {
        try {
          const vehicle =
            ride.vehicle ||
            (await this.vehicleModel.findById(ride.vehicleId).exec());
          const vType =
            (vehicle?.vehicleType as string)?.toLowerCase() || "car";
          const pickupCoords = ride.pickupLocation?.coordinates;
      ...
      const fullDetails = await this.buildFullRideDetailsPayload(updatedRide, {
        rideStatus: RideStatus.PICKUP,
        distanceInKm: remainingDistanceKm,
      });
      ...
      this.logger.log(
        `Passenger picked up for ride ${ride.rideUUId} by driver ${driverId}`,
      );
  ...
  async completeRide(
    ...
    try {
      ...
      if (ride.rideStatus !== RideStatus.ONGOING) {
        return {
          success: false,
          message: `Ride must be ONGOING to complete. Current: ${ride.rideStatus}`,
        };
      }
      ...
      const actualCompleteDurationInMinutes = Math.floor(
        (rideEndedAt - rideStartedAt) / (1000 * 60),
      );
      ...
      const distanceFare = Number(
        ride.fare?.distanceAmount || distanceInKm * perKmRate,
      );
      const totalFare =
        ride.fare?.totalAmount ||
        Number(Number(baseFareAmount) + Number(distanceFare));

      const discountAmount = Math.round(
        Number(ride.paymentDetails?.discountAmount || 0),
      );
      const finalAmount = Math.max(
        0,
        Math.round(Number(Number(totalFare) - Number(discountAmount))),
      );

      const commissionRate =
        Number(ride.paymentDetails?.driverCommission) || 0.2;
      ...
      if (!updatedRide) {
        return {
          success: false,
          message: "Failed to update ride to completed",
        };
      }
      ...
      await this.rideChannelService.publishRideCompleted(updatedRide.rideUUId, {
        ...
        fareBreakdown: {
          baseFare: Number(baseFareAmount),
          distanceCharge: Number(distanceCharge),
          discount: Number(discountAmount),
          totalFare: Number(finalAmount),
          subTotal: Number(existingFare.subTotal || 0),
          promocodeName: existingFare.promoCodeName || null,
        },
      ...
      const passenger = await this.userModel
        .findById(updatedRide.passengerId)
        .exec();
      if (passenger) {
        await this.notificationService.createNotification(
          {
            ...
            ablyChannelId:
              updatedRide.ablyChannelId ||
              `WG-RIDE-${updatedRide.rideUUId}-ride-details`,
            pickupLocation: updatedRide.pickupLocation,
            dropoffLocation: updatedRide.dropoffLocation,
            distanceInKm,
            estimatedTimeInMinutes: updatedRide.estimatedTimeInMinutes,
    ...
    try {
      ...
      if (!updated) {
        return {
          success: false,
          message:
            "Scheduled ride is not CONFIRMED (already transitioned or missing).",
        };
      }
      ...
      if (driverId) {
        await this.subscribeToDriverLocationChannel(driverId)
          .then(() => {
            this.logger.log(
              `Scheduled-ongoing: subscribed driver ${driverId} to location channel`,
            );
          })
          .catch((err: any) =>
            this.logger.warn(
      ...
      this.logger.log(
        `Scheduled ride ${updated.rideUUId} transitioned to ONGOING`,
      );
      return {
        success: true,
        message: "Scheduled ride transitioned to ONGOING successfully.",
      };
    } catch (err: any) {
      this.logger.error(
        `Failed to mark scheduled ride ${rideId} ONGOING: ${err?.message || err}`,
      );
      return {
        success: false,
        message: "Failed to mark scheduled ride ONGOING.",
      };
    }
  }
   ...
   */
  private async publishScheduledRideOngoingDetails(
    ride: RidesDocument,
  ): Promise<void> {
  ...
  async startScheduledRide(
    ...
    try {
      ...
      if (ride.rideStartedAt) {
        return {
          success: false,
          message: "Scheduled ride has already been started",
        };
      }
      ...
      await this.subscribeToDriverLocationChannel(driverId)
        .then(() => {
          this.logger.log(
            `Successfully subscribed driver ${driverId} to location channel on scheduled ride start`,
          );
        })
        .catch((err: any) =>
          this.logger.warn(
      ...
      this.logger.log(
        `Scheduled ride ${ride.rideUUId} started by driver ${driverId}`,
      );
      return { success: true, message: "Scheduled ride started successfully." };
    } catch (err: any) {
      this.logger.error(
        `Failed to start scheduled ride: ${err?.message || err}`,
      );
  ...
  async endScheduledRide(
    ...
    try {
      ...
      if (ride.rideType !== RideTypes.SCHEDULED) {
        return {
          success: false,
          message: "This mutation is only for SCHEDULED rides.",
        };
      }
      if (ride.rideStatus !== RideStatus.ONGOING) {
        return {
          success: false,
          message: `Scheduled ride must be ONGOING to end. Current: ${ride.rideStatus}`,
        };
      }
      if (ride.rideEndedAt) {
        return {
          success: false,
          message: "Scheduled ride has already been ended",
        };
      }
      ...
      const updatedRide = await this.ridesModel
        ...
        .exec();
      if (!updatedRide)
        return { success: false, message: "Failed to update scheduled ride" };
      ...
      this.logger.log(
        `Scheduled ride ${ride.rideUUId} ended and completed by driver ${driverId}`,
      );
      return {
        success: true,
        message: "Scheduled ride ended and completed successfully.",
      };
    } catch (err: any) {
  ...
  async cancelInstantRide(
    ...
    this.logger.log(
      `Passenger ${passengerId} cancelling instant ride ${rideId}`,
    );
    try {
      const ride = await this.ridesModel
        .findById(new Types.ObjectId(rideId))
      ...
      if (!ride) {
        this.logger.log(
          `Ride ${rideId} not found during cancellation - it was already deleted/cancelled. Treating as successful cancellation.`,
        );
      ...
      if (ride.rideType !== RideTypes.INSTANT) {
        return {
          success: false,
          message: "Only instant rides can be cancelled via this endpoint",
        };
      }
      ...
      if (
        ...
        return {
          success: false,
          message: `Cannot cancel ride in ${ride.rideStatus} status`,
        };
      }
      ...
      await this.ridesModel
        .findByIdAndUpdate(ride._id, {
          $set: { rideStatus: RideStatus.CANCELLED },
        })
        .exec();
      ...
      if (driverId && ride.rideStatus === RideStatus.CONFIRMED) {
        this.logger.log(
          `Ride ${rideUUId} was accepted by driver ${driverId}. Notifying driver about cancellation.`,
        );
        const driverUser = await this.userModel
          .findById(new Types.ObjectId(driverId))
          .exec();
        if (driverUser) {
          await this.notificationService.createNotification(
            {
              ...
              ablyChannelId:
                ride.ablyChannelId || `WG-RIDE-${rideUUId}-ride-details`,
              rideId: ride._id.toString(),
              cancelled: true,
              passengerId,
            } as any,
      ...
      if (pendingResolver) {
        this.logger.log(
          `Force-resolving pending driver response promise for ride ${rideUUId}`,
        );
        this.pendingDriverResponseResolvers.delete(rideUUId);
      ...
      this.logger.log(
        `Ride ${rideUUId} cancelled successfully by passenger ${passengerId}`,
      );
  ...
  async acknowledgeAndFinishRide(
    rideId: string,
    driverId: string,
  ): Promise<{ success: boolean; acknowledged: boolean; message: string }> {
    this.logger.log(
      `Driver ${driverId} acknowledging and finishing ride ${rideId}`,
    );
    try {
      const ride = await this.ridesModel
        .findById(new Types.ObjectId(rideId))
      ...
      if (
        !ride.rideEndedAt ||
        ride?.paymentDetails?.paymentStatus !== PaymentStatusEnum.PAID
      ) {
        return {
          success: false,
          message: `Ride must be ended and payment should be confirmed to acknowledge and finish. Current: ${ride.rideStatus} ${ride?.paymentDetails?.paymentStatus}`,
          acknowledged: false,
        };
      }
      if (ride.isAcknowledgeByDriver) {
        return {
          success: false,
          message: "Ride has already been acknowledged by driver",
          acknowledged: true,
        };
      }
      ...
      const driver = await this.userDetailsModel
        .findOneAndUpdate(
      ...
      if (!driver) {
        this.logger.debug("Driver not found");
      } else {
        this.logger.debug(
          `Driver rides: ${driver.totalRidesAsDriver - 1} → ${driver.totalRidesAsDriver}`,
        );
      }
      ...
      this.logger.log(
        `Ride ${rideId} acknowledged and finished by driver ${driverId}`,
      );
      return {
        success: true,
        acknowledged: true,
        message: "acknowledged the payment successfully",
      };
    } catch (err: any) {
      this.logger.error(
        `Failed to acknowledge and finish ride: ${err?.message || err}`,
      );
      return {
        success: false,
        acknowledged: false,
        message: "Failed to acknowledge and finish ride",
      };
    }
  }
  ...
  async getVehicleEstimates(params: {
    ...
    if (params.promoCodeId) {
      const promoResult = await this.loadAndValidatePromoCode(
        params.promoCodeId,
        params.passengerId,
      );
    ...
    try {
      ...
      this.logger.warn(
        `getVehicleEstimates: Baato pickup-to-dropoff distance failed, using default fallback (${routeDistanceKm} km): ${err?.message || err}`,
      );
    }

    const estimates = await Promise.all(
      vehicleTypes.map(async (type): Promise<VehicleEstimateGraphQL | null> => {
        try {
          ...
          if (!nearestDriver) {
            this.logger.log(
              `No available driver for vehicle type ${type}; skipping estimate`,
            );
            return null;
          }
          this.logger.log(
            `Nearest driver for ${type}: distance: ${nearestDriver.distanceKm} km, ETA: ${nearestDriver.durationMinutes} min`,
          );
          ...
          if (!discountInfo && promo && !promoCodeMessage) {
            if (
              Number(promo.minimumFare) > 0 &&
              originalFare < Number(promo.minimumFare)
            ) {
          ...
          return {
            ...
            estimatedFare: Math.round(
              originalFare - (discountInfo?.discountAmount || 0),
            ),
            originalFare,
            discountAmount: discountInfo?.discountAmount || 0,
            promoCodeName:
              discountInfo?.promoCodeName ||
              (effectivePromoCodeMessage ? promoCodeName : undefined),
            promoCodeId: discountInfo?.promoCodeId
              ? discountInfo.promoCodeId.toString()
              : promoCodeIdValue || undefined,
          ...
          this.logger.error(
            `Failed to calculate vehicle estimate for type ${type}: ${err?.message || err}${err.response ? `, response: ${JSON.stringify(err.response)}` : ""}`,
          );
          return null;
        }
      }),
    ...
    return estimates.filter(
      (estimate): estimate is VehicleEstimateGraphQL => estimate !== null,
    );
  }
  ...
  private async loadAndValidatePromoCode(
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          `Promo code ${promo.name} is not in a valid time/status window for estimates`,
        );
        return {
          promo,
          reason: `Promo code '${promo.name}' is expired or not active`,
        };
      }

      if (
        promo.appliedTo !== AppliedToEnum.ALL_RIDES &&
        promo.appliedTo !== AppliedToEnum.INSTANT
      ) {
        this.logger.warn(
          `Promo code ${promo.name} is not applicable to INSTANT rides`,
        );
        return {
          promo,
          reason: `Promo code '${promo.name}' is not applicable to instant rides`,
        };
      }
      ...
      const [totalUsage, userUsage] = await Promise.all([
        this.promoCodeUsedModel
          .countDocuments({ promoCodeId: promo._id })
          .exec(),
        this.promoCodeUsedModel
          .countDocuments({
            userId: toMongoId(passengerId),
      ...
      if (totalUsage >= promo.totalUsageLimit) {
        this.logger.warn(
          `Promo code ${promo.name} has reached total usage limit`,
        );
        return {
          promo,
          reason: `Promo code '${promo.name}' has reached its usage limit`,
        };
      }
      if (userUsage >= promo.perUserLimit) {
        this.logger.warn(
          `Promo code ${promo.name} has reached per-user limit for ${passengerId}`,
        );
        return {
          promo,
          reason: `Promo code '${promo.name}' usage limit reached for this user`,
        };
      }
      ...
      this.logger.warn(
        `Failed to validate promo code for estimates: ${err?.message || err}`,
      );
  ...
  private applyPromoDiscountToFare(
    ...
    if (
      ...
      this.logger.warn(
        `Promo code ${promo.name} minimum fare not met: ${originalFare} < ${promo.minimumFare}`,
      );
      return null;
    }
    ...
    if (promo.discountType === DiscountTypeEnum.PERCENTAGE) {
      discount = Math.round(
        originalFare * ((Number(promo.percentageAmount) || 0) / 100),
      );
  ...
  private async findNearestDriverDistance(
    ...
    try {
      ...
      let nearest: { distanceKm: number; durationMinutes: number } | null =
        null;
      for (const v of validVehicles) {
        const driver = v.driverId as any as UserDocument;
      ...
      this.logger.log(
        `Nearest driver for ${vehicleType}: distance: ${nearest.distanceKm} km, ETA: ${nearest.durationMinutes} min`,
      );
      return {
      ...
      this.logger.warn(
        `findNearestDriverDistance failed for ${vehicleType}: ${err?.message || err}`,
      );
      return null;
    }
  }
  ...
  async cancelRideNotification(
    ...
    try {
      ...
      const userDetails = await this.userDetailsModel
        .findOne({ userId: toMongoId(userId) })
        .exec();
      const userSnapShot = {
      ...
      await this.rideChannelService.publishRideEvent(
        ...
        "ride-cancelled",
        {
          ...
          cancelledAt:
            ride.cancellationDetail?.cancelledAt || new Date().toISOString(),
      ...
      return {
        success: false,
        message: "Send notification and ably to ride successfully",
      };
    } catch (err: any) {
      this.logger.error(
        `Failed to send notification and ably to  ride: ${err?.message || err}`,
      );
      return {
        success: false,
        message: "Failed to send notification and ably to ride",
      };
    }
  }
  ...
  private async buildAcceptDetails(
    ...
    return {
      ...
      passenger: {
        passengerId: ride.passengerId.toString(),
        fullName:
          passengerDetails?.fullName || passengerUser?.fullName || "Passenger",
        phone: passengerUser?.phone || "",
        profileImage: getActiveProfileImageUrl(
          passengerDetails?.profileImages,
          (key) => this.s3.getPublicUrl(key),
  ...
  private async buildScheduledAcceptDetails(
    ...
    const [driverUser, driverDetails, vehicle, passengerUser, availabilityDoc] =
      await Promise.all([
        this.userModel.findById(new Types.ObjectId(driverId)).exec(),
        this.userDetailsModel
          .findOne({ userId: new Types.ObjectId(driverId) })
    ...
    const resolvedAvailability = ride.bookingTime
      ? this.resolveAvailabilityDay(
          availabilityDoc,
          ride.bookingTime,
          noOfPassengers,
    ...
    const availability = resolvedAvailability
      ? this.buildScheduledAvailabilityInfo(
          resolvedAvailability.day,
          resolvedAvailability.effectiveSeats,
          undefined,
          undefined,
          ride.bookingTime,
        )
  ...
  private async buildFullRideDetailsPayload(
    ...
    const passengerUser = await this.userModel
      ...
      .exec();
    const passengerDetails = await this.userDetailsModel
      .findOne({ userId: ride.passengerId, deleted: false })
      .exec();
    let driver: import("@libs/services/ably").RideDetailsPayload["driver"] =
      undefined;
    if (ride.driverId) {
      const driverUser = await this.userModel.findById(ride.driverId).exec();
      const driverDetails = await this.userDetailsModel
        .findOne({ userId: ride.driverId, deleted: false })
        .exec();
      driver = {
    ...
    let vehicle: import("@libs/services/ably").RideDetailsPayload["vehicle"] =
      undefined;
    const vehicleDoc =
      ride.vehicle || (await this.vehicleModel.findById(ride.vehicleId).exec());
    if (vehicleDoc) {
