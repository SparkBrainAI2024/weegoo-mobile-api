...
export class MatchmakingService {
  ...
  private async findAvailableScheduledDrivers(
    ...
    for (const v of vehicles) {
      ...
      const bufferMinutes = day.pickupBufferTimeMinutes || 0;
      const conflictingRide = (
        rideTimesByDriver.get(driver._id.toString()) || []
      ).some(
    ...
    this.logger.log(
      `Found ${drivers.length} available scheduled drivers for the requested destination`,
    );
    return drivers;
  }
                ...
                .findByIdAndUpdate(updatedRide._id, {
                  $set: {
                    schedule: {
                      ...
                      timeSlots: (resolved.day.timeSlots || []).map((s) => ({
