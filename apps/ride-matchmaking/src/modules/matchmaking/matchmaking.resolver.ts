...
export class MatchmakingResolver {
  ...
  @Mutation(() => ScheduledMatchResultGraphQL, {
    ...
    description:
      "BOOKING flow for a SCHEDULED ride. Returns the drivers/vehicles available for the requested day (JEEP/CAR/MICRO), prioritised by the availability time slot that matches the requested booking time most closely and then by the nearest availability pickup location, with vehicle + availability + driver info. The ride is kept BOOKING; a driver may accept any time before the pickup buffer.",
  })
