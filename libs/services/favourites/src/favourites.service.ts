...
export class FavouriteService {
     ...
     */
    async findRides(user: User, options: PaginationInputOnly) {
        return this.favouriteRepository.findByPassengerIdWithPagination(
            user._id.toString(),
            options,
        );
    }
    ...
    async createFavorite(
        rideData: CreateFavouriteInput,
        passengerId: string,
    ): Promise<FavouritesDocument> {
        const ride = await this.ridesRepository.findByIdWithVehicle(
            rideData.rideId,
            passengerId,
        );
        if (!ride) {
        ...
        if (
            ...
            throw ErrorException(null, "RIDES.RIDE_STATUS_MISMATCHED", 400);
        }

        const favorites = await this.favouriteRepository.createFavourite({
