...
export class VehicleService {
  ...
  async registerVehicle(
    ...
    try {
      const existing = await this.vehicleRepository.findByNumberPlate(
        input.numberPlate,
      );
      if (existing) {
        ErrorException(
          null,
          "VEHICLE.NUMBER_PLATE_ALREADY_EXISTS",
  ...
  async editVehicle(
    ...
    if (
      !vehicleExists ||
      vehicleExists.driverId.toString() !== driverId.toString()
    ) {
      ...
      for (const vehicle of vehicles) {
        ...
        for (const img of inactiveImages) {
          try {
            ...
            this.logger.error(`Failed to delete S3 key: ${img.s3Key}`, e);
          }
        }
