...
export class NotificationRepository extends BaseRepository<NotificationDocument> {
  constructor(
    ...
    private readonly _model: BaseModel<NotificationDocument>,
  ) {
    super(_model);
  }

  createNotification(
    notificationData: Partial<NotificationDocument>,
  ...
  ): Promise<NotificationListWithCursorPaginationResponse> {
    try {
      const filter = { userId: toMongoId(userId), deleted: false };
      const { data, nextCursor, hasNextPage } = await this.cursorPaginate(
        filter,
        pagination,
      );
      return {
